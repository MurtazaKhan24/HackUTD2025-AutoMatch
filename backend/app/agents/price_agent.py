...
def get_target_listing_price(req: PriceRequest) -> PriceResponse:
    ...
    system_prompt = (
        "/no_think"
        "NO THINKING. NO EXPLANATION. Respond with ONLY the JSON object, nothing else. "
        "Given budget, payment method, and location, compute a recommended target_listing_price "
        ...
        "Do not use any other field names. NO EXTRA TEXT."
    )
    user_content = {
        "budget": req.budget,
        "payment": req.payment,
    ...
    try:
        if isinstance(response, dict):
            ...
            if 'error' in response:
                ...
                return PriceResponse(
                    target_listing_price=0, 
                    assumptions={"error": "LLM request failed", "details": str(response.get('error'))}
                )
            # Otherwise it should be the parsed JSON response
        ...
        return PriceResponse(
            target_listing_price=0, 
            assumptions={"error": "Failed to parse LLM response", "raw": str(response)}
        )
